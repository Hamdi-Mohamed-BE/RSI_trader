...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$patterns = @('*-m app.telegram_signaler*','*-m app.automation*','*-m app.orb_bot*','*-m app.challenge_20pip*','*-m app.bpr_bot*','*-m app.strategy_bot_worker*','*sniper_entry_bot.py*--loop*','*run_telegram_signaler.bat*','*run_automation.bat*','*run_lta_bot.bat*','*run_orb_bot.bat*','*run_20pip_bot.bat*','*run_20pip_challenge.bat*','*run_bpr_bot.bat*','*run_grid_bot.bat*','*run_trend_bot.bat*','*run_mean_reversion_bot.bat*','*run_dca_bot.bat*','*run_news_bot.bat*','*run_arbitrage_bot.bat*','*run_sniper_bot.bat*');" ^
  "$stopped = @();" ^
  ...
  "Remove-Item -LiteralPath (Join-Path $root 'reports\telegram_signaler\telegram_signaler.lock'),(Join-Path $root 'reports\telegram_signaler\heartbeat.json'),(Join-Path $root 'reports\automation\automation.lock'),(Join-Path $root 'reports\automation\automation_heartbeat.json'),(Join-Path $root 'reports\orb_bot\orb.lock'),(Join-Path $root 'reports\orb_bot\orb_heartbeat.json'),(Join-Path $root 'reports\20pip_challenge\challenge.lock'),(Join-Path $root 'reports\20pip_challenge\challenge_heartbeat.json'),(Join-Path $root 'reports\bpr_bot\bpr.lock'),(Join-Path $root 'reports\bpr_bot\bpr_heartbeat.json') -Force -ErrorAction SilentlyContinue;" ^
