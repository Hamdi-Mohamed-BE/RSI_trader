...
async def lifespan(app: FastAPI):
    ...
    with Session(engine) as session:
        ...
        if db_copier_enabled is None:
            ...
            SettingsService.set(session, "break_even_trigger_tp_level", settings.BREAK_EVEN_TRIGGER_TP_LEVEL)
            SettingsService.set(session, "split_legs_enabled", settings.SPLIT_LEGS_ENABLED)
            SettingsService.set(session, "split_legs_max_count", settings.SPLIT_LEGS_MAX_COUNT)
