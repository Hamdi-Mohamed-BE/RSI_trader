...
Use this module as an extra validation layer during deep scans, A-only pre-order scans, and whenever the user shows or mentions a `Signal Pro` BUY/SELL marker from TradingView.
...
- Slow EMA: `21`
- RSI: `14`
- RSI overbought filter: `70`
- RSI oversold filter: `30`
- ATR: `14`
- SL distance: `1.0 x ATR`
- TP distance: `2.0 x ATR`
- Buy signal: EMA9 crosses above EMA21 and RSI is below `70`.
- Sell signal: EMA9 crosses below EMA21 and RSI is above `30`.
- Valid buy SL: signal candle low minus `1 x ATR`; valid buy TP: signal candle close plus `2 x ATR`.
- Valid sell SL: signal candle high plus `1 x ATR`; valid sell TP: signal candle close minus `2 x ATR`.

How to use it:

- Only trust a Signal Pro marker after the candle is closed. Do not treat an intrabar marker as confirmed.
- A fresh Signal Pro marker is strongest on the current or previous `1-3` closed candles. Older markers are context only unless price retests the signal area and confirms again.
- Run it on the execution timeframe first:
  - XAUUSD and crypto: `1m` and `5m`, with `15m` context.
  - Forex: `5m`, with `15m` and `H1` context.
- Do not take Signal Pro alone. Require at least one structure confirmation: break/retest, sweep/reclaim, higher-low/lower-high, VWAP/EMA reclaim/rejection, or clean momentum continuation.
- If Signal Pro agrees with RSI divergence, EMA/VWAP structure, and M15/H1 context, upgrade confidence.
- If Signal Pro conflicts with RSI divergence or higher-timeframe structure, downgrade confidence and wait for a cleaner trigger.
- If BUY and SELL markers appear close together, classify the market as choppy/noisy and avoid new A-grade entries until structure clears.
- If the Signal Pro TP/SL is too tight for spread/broker stop distance, adjust the plan using the normal TP/SL rules and say it was adjusted after validation.
- If the Signal Pro SL is too wide for a clean intraday setup, do not force the trade; wait for a retest or smaller invalidation.
- For A-only AI pre-orders, Signal Pro can improve a setup grade but cannot make a weak structure setup A by itself.
- For copied signals, label the source as `signal copy`; if Signal Pro agrees, write `reason: signal copy, Signal Pro confirms direction after AI validation`.

Validation checklist when using Signal Pro:

- Confirm latest closed candle created the crossover, not the still-forming candle.
- Check RSI is not already too late: avoid late buys when RSI is above `70-75`, avoid late sells when RSI is below `25-30`.
- Check spread is small compared with ATR and TP1 distance.
- Check entry is not in the middle of a range.
- Check no opposite RSI divergence or fresh opposite Signal Pro marker appeared.
- Check higher timeframe does not strongly reject the direction.

In output, mention Signal Pro only when it matters:

reason: Signal Pro M5 buy confirmed after EMA9/EMA21 cross, RSI valid, and price held above the breakout.
