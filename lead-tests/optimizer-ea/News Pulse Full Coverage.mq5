...
input int    InpEntryAnchorMode=0;              // 0=current Ask/Bid; 1=forming M1 high/low
input double InpEntryOffsetPrice=12.0;         // buy above Ask / sell below Bid
input double InpStopLossPrice=10.0;            // one R
input bool   InpUseTakeProfit=false;            // research switch: optional fixed-R target
input double InpTakeProfitR=2.0;                // target distance in initial stop-risk units
input bool   InpUseTrailingStop=true;           // research switch: current release uses true
...
input bool   InpCancelOppositeAfterFill=false;  // false preserves the current non-OCO behavior
...
bool NP_FindTesterEvent(datetime &event_time,long &event_id,string &kind)
{
   ...
   string best_kind="";
   int total=NP_GeneratedCalendarEventCount();
   int offset=AAA_ServerOffsetSeconds();
...
bool NP_SendStraddle(const datetime event_time,const long event_id,const string kind)
{
   ...
   double buy_anchor=tick.ask;
   double sell_anchor=tick.bid;
   if(InpEntryAnchorMode==1)
   {
      double active_high=iHigh(_Symbol,PERIOD_M1,0);
      double active_low=iLow(_Symbol,PERIOD_M1,0);
      if(active_high<=0.0 || active_low<=0.0) return false;
      // MT5 bars are Bid-based. Keep both stops valid relative to the live
      // Ask/Bid while allowing the forming M1 extremes to move them farther out.
      buy_anchor=MathMax(tick.ask,active_high);
      sell_anchor=MathMin(tick.bid,active_low);
   }
   double buy_entry=AAA_Price(_Symbol,buy_anchor+InpEntryOffsetPrice);
   double sell_entry=AAA_Price(_Symbol,sell_anchor-InpEntryOffsetPrice);
   ...
   double buy_tp=(InpUseTakeProfit ? AAA_Price(_Symbol,buy_entry+InpTakeProfitR*InpStopLossPrice) : 0.0);
   double sell_tp=(InpUseTakeProfit ? AAA_Price(_Symbol,sell_entry-InpTakeProfitR*InpStopLossPrice) : 0.0);
   ...
   if(allow_buy)
   {
      buy_ok=AAA_Trade.BuyStop(buy_lots,buy_entry,_Symbol,buy_sl,buy_tp,ORDER_TIME_SPECIFIED,expiry,prefix+"B");
      if(!buy_ok)
         Print("News Pulse: buy-stop placement failed: ",AAA_Trade.ResultRetcodeDescription());
   }
   if(allow_sell)
   {
      sell_ok=AAA_Trade.SellStop(sell_lots,sell_entry,_Symbol,sell_sl,sell_tp,ORDER_TIME_SPECIFIED,expiry,prefix+"S");
      if(!sell_ok)
...
void NP_ManageLifecycle()
{
   ...
   if(has_position)
   {
      if(InpCancelOppositeAfterFill) NP_DeletePendingOrders();
      NP_TrailPositions();
   }
...
int OnInit()
{
   ...
   if((!InpEnableBuySide && !InpEnableSellSide) ||
      ...
      (InpEntryAnchorMode!=0 && InpEntryAnchorMode!=1) ||
      InpEntryOffsetPrice<=0.0 || InpStopLossPrice<=0.0 ||
      (InpUseTakeProfit && InpTakeProfitR<=0.0) ||
   ...
   Print("News Pulse XAU optimization harness v2.16 loaded on ",_Symbol,
