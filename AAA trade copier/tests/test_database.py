...
def test_create_schema_upgrades_existing_copy_test_history(tmp_path: Path) -> None:
    ...
    with engine.begin() as connection:
        ...
        connection.execute(
            text("CREATE TABLE risk_profiles (id VARCHAR(36) PRIMARY KEY, name VARCHAR(120))")
        )
    ...
    risk_columns = {column["name"] for column in inspect(engine).get_columns("risk_profiles")}
