...
def chart(stage: str, rows: list[dict]) -> Path:
    ...
    fig, axes = plt.subplots(rows_count, columns, figsize=(15, 4.6 * rows_count), dpi=160, squeeze=False)
    for axis, group in zip(axes.flat, groups):
        for row in [item for item in rows if item["group"] == group]:
            ...
            axis.plot(range(len(y)), y, linewidth=1.25, label=row["case"])
    ...
    for axis in axes.flat[len(groups):]:
        axis.axis("off")
    fig.suptitle(f"ETH Top Down FVG — {stage} focused pipeline")
    fig.tight_layout(rect=(0, 0, 1, 0.97))
    output = ROOT / f"ETH TOP DOWN FVG - {stage.upper()} CONFIG COMPARISON.png"
