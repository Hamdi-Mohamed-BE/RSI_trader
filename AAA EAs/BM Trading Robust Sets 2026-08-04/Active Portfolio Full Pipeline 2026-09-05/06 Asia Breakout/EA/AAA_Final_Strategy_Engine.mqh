...
input ENUM_TIMEFRAMES InpAsiaSignalTimeframe=PERIOD_H1;
input int    InpAsiaEntryStartUTC=8;
input int    InpAsiaEntryEndUTC=13;
input int    InpAsiaStopMode=0; // 0=range midpoint, 1=opposite range edge, 2=signal candle
input double InpAsiaStopBufferPercent=0.00;
input bool   InpAsiaUseNativeTrailing=true;
input double InpAsiaTrailStartR=2.0;
input double InpAsiaTrailDistanceR=0.5;
...
{
   if(InpUseTrailing && InpAsiaUseNativeTrailing)
      AAA_TrailR(_Symbol,InpMagic,InpAsiaTrailStartR,InpAsiaTrailDistanceR);
   if(!AAA_NewBar(_Symbol,InpAsiaSignalTimeframe,g_last_bar) || !InpEnableTrading || !AAA_SpreadOK()) return;
   ...
   if(utc.hour<InpAsiaEntryStartUTC || utc.hour>InpAsiaEntryEndUTC) return;
   double high,low;
   if(!AAA_SessionRangeUTC(_Symbol,PERIOD_M15,0,8,high,low)) return;
   MqlRates r[]; if(!AAA_LoadRates(InpAsiaSignalTimeframe,3,r)) return;
   double buffer=(high-low)*InpAsiaBufferPercent;
   double midpoint=(high+low)/2.0;
   double stop_buffer=(high-low)*InpAsiaStopBufferPercent;
   double long_stop=midpoint;
   double short_stop=midpoint;
   if(InpAsiaStopMode==1)
     {
      long_stop=low-stop_buffer;
      short_stop=high+stop_buffer;
     }
   else if(InpAsiaStopMode==2)
     {
      long_stop=r[1].low-stop_buffer;
      short_stop=r[1].high+stop_buffer;
     }
   if(r[1].close>high+buffer && r[1].low<=high+buffer && AAA_MarkovAllowsDirection(1))
      AAA_SendMarket(_Symbol,1,long_stop,InpRewardRisk,InpRiskPercent,InpMagic,"AAA Asia confirmed retest");
   else if(r[1].close<low-buffer && r[1].high>=low-buffer && AAA_MarkovAllowsDirection(-1))
      AAA_SendMarket(_Symbol,-1,short_stop,InpRewardRisk,InpRiskPercent,InpMagic,"AAA Asia confirmed retest");
}
