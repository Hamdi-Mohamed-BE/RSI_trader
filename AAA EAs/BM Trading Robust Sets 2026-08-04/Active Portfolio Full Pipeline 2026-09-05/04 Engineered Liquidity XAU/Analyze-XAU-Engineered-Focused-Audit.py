...
def chart(stage: str, rows: list[dict]) -> Path:
    ...
    fig, axes = plt.subplots(rows_count, columns, figsize=(15, 4.4 * rows_count), dpi=160, squeeze=False)
    for axis, group in zip(axes.flat, groups):
        for row in [item for item in rows if item["group"] == group]:
            ...
            axis.plot(range(len(y)), y, linewidth=1.2, label=row["case"])
    ...
    for axis in axes.flat[len(groups):]:
        axis.axis("off")
    fig.suptitle(f"Engineered Liquidity XAU — {stage} focused pipeline")
    fig.tight_layout(rect=(0, 0, 1, 0.97))
    output = ROOT / f"ENGINEERED LIQUIDITY XAU - {stage.upper()} CONFIG COMPARISON.png"
...
if __name__ == "__main__":
    ...
    simple = [{
        "case": row["case"], "group": row["group"], "return_pct": row["return_pct"],
        "pf": row["profit_factor"], "win_rate": row["win_rate_pct"], "dd": row["max_drawdown_pct"],
        "trades": row["trades"], "sharpe": row["sharpe_ratio"], "recovery": row["recovery_factor"],
    } for row in data]
