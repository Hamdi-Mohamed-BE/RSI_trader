...
def chart(stage: str, rows: list[dict]) -> Path:
    ...
    for axis, group in zip(axes.flat, groups):
        for row in [item for item in rows if item["group"] == group]:
            ...
            axis.plot(range(len(values)), values, linewidth=1.2, label=row["case"])
    ...
    for axis in axes.flat[len(groups):]:
        axis.axis("off")
    fig.suptitle(f"ORB Volume Profile XAU — {stage} focused pipeline")
    fig.tight_layout(rect=(0, 0, 1, 0.97))
    output = ROOT / f"ORB VOLUME PROFILE XAU - {stage.upper()} CONFIG COMPARISON.png"
