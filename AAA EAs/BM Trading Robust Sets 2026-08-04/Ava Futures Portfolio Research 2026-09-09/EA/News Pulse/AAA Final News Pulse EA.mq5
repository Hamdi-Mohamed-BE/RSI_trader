...
{
   // Exact release epochs are generated from FXMacroData. Manual date/time
   // assumptions and silent partial schedules are forbidden in Strategy Tester.
...
bool NP_SendStraddle(const datetime event_time,const long event_id,const string kind)
{
   ...
   if(!AvaPortfolioTryAcquire(_Symbol,InpMagic))
     {
      Print("News Pulse: another Calyx EA currently owns this netting contract.");
      return false;
     }
   AAA_Trade.SetExpertMagicNumber((ulong)InpMagic);
   ...
   if(!buy_ok && !sell_ok)
     {
      AvaPortfolioReleaseIfIdle(_Symbol,InpMagic);
      return false;
     }
