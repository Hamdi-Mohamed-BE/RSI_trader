...
double AAA_Volume(const string symbol,const double raw)
{
   ...
   if(minimum<=0.0 || step<=0.0) return 0.0;
   return NormalizeDouble(minimum,8); // Ava Futures: always one broker-minimum contract.
}
...
bool AAA_SendMarket(const string symbol,const int direction,const double stop,const double reward_risk,const double risk_percent,const long magic,const string comment)
{
   ...
   if(!AvaPortfolioTryAcquire(symbol,magic)) return false;
   bool sent=(direction>0 ? AAA_Trade.Buy(lots,symbol,0.0,sl,tp,comment)
                          : AAA_Trade.Sell(lots,symbol,0.0,sl,tp,comment));
   if(!sent) AvaPortfolioReleaseIfIdle(symbol,magic);
   return sent;
}
...
bool AAA_SendPending(const string symbol,const ENUM_ORDER_TYPE type,const double entry,const double stop,const double reward_risk,const double risk_percent,const long magic,const datetime expiry,const string comment)
{
   ...
   if(!AvaPortfolioTryAcquire(symbol,magic)) return false;
   bool sent=false;
   if(type==ORDER_TYPE_BUY_STOP) sent=AAA_Trade.BuyStop(lots,price,symbol,sl,tp,ORDER_TIME_SPECIFIED,expiry,comment);
   else if(type==ORDER_TYPE_SELL_STOP) sent=AAA_Trade.SellStop(lots,price,symbol,sl,tp,ORDER_TIME_SPECIFIED,expiry,comment);
   else if(type==ORDER_TYPE_BUY_LIMIT) sent=AAA_Trade.BuyLimit(lots,price,symbol,sl,tp,ORDER_TIME_SPECIFIED,expiry,comment);
   else if(type==ORDER_TYPE_SELL_LIMIT) sent=AAA_Trade.SellLimit(lots,price,symbol,sl,tp,ORDER_TIME_SPECIFIED,expiry,comment);
   if(!sent) AvaPortfolioReleaseIfIdle(symbol,magic);
   return sent;
}
