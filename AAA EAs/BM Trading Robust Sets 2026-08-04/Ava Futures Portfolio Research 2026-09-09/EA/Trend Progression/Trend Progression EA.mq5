...
double NormalizeVolume(const double requested)
  {
   ...
   return NormalizeDouble(minimum,VolumeDigits(step)); // Ava Futures: one minimum contract.
  }
...
void ProcessNewBar()
  {
   ...
   if(!AvaPortfolioTryAcquire(_Symbol,InpMagic)) return;
   bool sent=(direction>0 ? trade.Buy(volume,_Symbol,0.0,stop,take_profit,"Trend progression long")
                          : trade.Sell(volume,_Symbol,0.0,stop,take_profit,"Trend progression short"));
   if(!sent) AvaPortfolioReleaseIfIdle(_Symbol,InpMagic);
  }
