...
bool PlaceSignal(const TradeSignal &signal)
{
   ...
   bool ok = false;
   if(!AvaPortfolioTryAcquire(_Symbol, InpMagicNumber))
      return false;
   if(signal.dir > 0)
   ...
   if(ok)
      ...
      AvaPortfolioReleaseIfIdle(_Symbol, InpMagicNumber);
...
double NormalizeVolume(const double volume)
{
   ...
   return NormalizeDouble(min_vol, digits); // Ava Futures: one broker-minimum contract.
}
