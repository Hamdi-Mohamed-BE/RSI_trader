#property copyright "Nasdaq 5M Open EMA ATR research EA"
...
void ProcessNewBar()
{
   ...
   if(rates[1].close>ema && InpAllowLong) SendEntry(1,atr);
   else if(rates[1].close<ema && InpAllowShort) SendEntry(-1,atr);
}

int OnInit()
{
   if(InpSignalTimeframe!=PERIOD_M5 || InpEMAPeriod<2 || InpATRPeriod<2 ||
