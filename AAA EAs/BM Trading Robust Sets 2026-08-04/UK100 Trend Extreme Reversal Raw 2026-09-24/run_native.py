"""Native MT5 Strategy Tester raw test: UK 100 trend + extreme-move reversal, all option combinations.
...
Usage: python run_native.py            # all cases
       python run_native.py H4         # only cases whose name starts with this prefix
"""
...
def cases() -> list[dict]:
    ...
    for tf, t in CONFIG["timeframes"].items():
        for tk, tv in CONFIG["trend"].items():
            for xk, xv in CONFIG["extreme"].items():
                for ek, ev in CONFIG["exit"].items():
                    name = f"{tf}-{tk}-{xk}-{ek}"
                    inputs = dict(CONFIG["inputs"], InpTrendMode=tv, InpExtremeMode=xv, InpExitMode=ev,
                                  InpSignalTimeframe=t["enum"], InpCase=f"ter-{name}")
                    out.append(dict(name=name, period=t["period"], tf=tf, trend=tk, extreme=xk, exit=ek, inputs=inputs))
    return out


def run_case(c: dict) -> dict:
    start, end = CONFIG["start_date"], CONFIG["end_date"]
    case = f"ter-{c['name']}-5y-m{CONFIG['model']}-d{CONFIG['execution_delay_ms']}"
    out = OUT / case
    out.mkdir(parents=True, exist_ok=True)
    set_name = f"ter-{c['name']}.set"
    ...
    checks = {
        ...
        "inputs_match": all(f"{k}={i[k]}" in body for k in ("InpTrendMode", "InpExtremeMode", "InpExitMode", "InpSignalTimeframe")),
    }
    flags = {k: len(re.findall(p, journal, re.I)) for k, p in {
        ...
        "rejected": r"TER_REJECT|invalid (stops|volume|price)",
        "market_closed": r"market closed",
        "critical": r"critical|access violation",
    }.items()}
    meta = dict(case=case, name=c["name"], tf=c["tf"], trend=c["trend"], extreme=c["extreme"], exit=c["exit"],
                ...
                ea_summary=re.findall(r"TER_SUMMARY\|[^\r\n]*", journal)[-1:],
                ea_spec=re.findall(r"TER_SPEC\|[^\r\n]*", journal)[-1:],
