...
#property description "Tester-only D14 H1 M5 structure research; explicit bounded optimization inputs"
#include <Trade/Trade.mqh>
input double InpRiskPercent=1.0;
input long InpMagic=84133001;
input int InpDeviationPoints=30;
input int InpH1Pivot=2;
input int InpM5Pivot=2;
input int InpZoneMode=0; // 0 broken wick, 1 entire origin candle
input int InpWickMode=0; // 0 whole body outside, 1 close outside
input int InpDirection=0; // 0 both, 1 long, -1 short
input int InpHoldBars=3;
input double InpStopBufferATR=0;
input double InpTargetR=2;
input int InpManagement=0; // 0 off, 1 BE at 1R, 2 fixed-R trail, 3 structural trail
input int InpMaxHoldHours=0;
input int InpMaxZoneHours=0;
...
double initialRiskDistance=0,spreadCost=0;
...
void Audit(string event,string detail,double entry=0,double sl=0,double tp=0,double lots=0,double risk=0)
{
 ...
 FileWrite(audit,Stamp(TimeCurrent()),event,detail,bias,zoneDir,Stamp(zoneOrigin),Stamp(zoneCreated),zoneLow,zoneHigh,
           ...
           entry,sl,tp,lots,risk,AccountInfoDouble(ACCOUNT_EQUITY),spreadCost);
 FileFlush(audit);
}
...
bool IsPivot(MqlRates &b[],bool high,int span)
{
 int c=ArraySize(b)-1-span;if(c<span)return false;
 for(int j=1;j<=span;j++)
 {
  if(high && (b[c].high<=b[c-j].high || b[c].high<=b[c+j].high))return false;
  if(!high && (b[c].low>=b[c-j].low || b[c].low>=b[c+j].low))return false;
 }
 return true;
}
bool Rejection(MqlRates &b,int dir,double lo,double hi)
{
 if(InpWickMode==0)return Reject(b,dir,lo,hi);
 return dir>0?(b.low>=lo && b.low<=hi && b.close>hi):(b.high<=hi && b.high>=lo && b.close<lo);
}
...
void SetZone(int dir,MqlRates &origin,datetime created)
{
 ...
 if(InpZoneMode==1){zoneLow=origin.low;zoneHigh=origin.high;}
...
void ReadHour()
{
 ...
 int n=2*InpH1Pivot+1,c=InpH1Pivot,k=n-1;
 MqlRates h[];if(CopyRates(_Symbol,PERIOD_H1,1,n,h)!=n)return;
 if(h[k].time+3600>TimeCurrent())return;
 lastH1=t;
 if(IsPivot(h,true,InpH1Pivot))hHigh=h[c];if(IsPivot(h,false,InpH1Pivot))hLow=h[c];
 if(h[k].time+3600<firstTick)return;
 if(bias>0 && hHigh.time>0 && hLow.time>hHigh.time && h[k].close>hHigh.high && h[k-1].close<=hHigh.high && hHigh.time!=seenBuyOrigin)
 {seenBuyOrigin=hHigh.time;SetZone(1,hHigh,h[k].time+3600);}
 if(bias<0 && hLow.time>0 && hHigh.time>hLow.time && h[k].close<hLow.low && h[k-1].close>=hLow.low && hLow.time!=seenSellOrigin)
 {seenSellOrigin=hLow.time;SetZone(-1,hLow,h[k].time+3600);}
}
...
void SendEntry(bool buy)
{
 ...
 double extra=0;
 if(InpStopBufferATR>0){double a[];if(CopyBuffer(atrHandle,0,1,1,a)!=1){errors++;Audit("error","ATR unavailable");return;}extra=InpStopBufferATR*a[0];}
 double entry=buy?tick.ask:tick.bid,sl=RoundPrice(buy?mLow.low-Step()-extra:mHigh.high+Step()+extra,!buy);
 double distance=buy?entry-sl:sl-entry;
 double tp=RoundPrice(buy?entry+InpTargetR*distance:entry-InpTargetR*distance,buy);
 ...
 double spreadPnl=0;if(!OrderCalcProfit(ORDER_TYPE_BUY,_Symbol,lots,tick.ask,tick.bid,spreadPnl)){errors++;Audit("error","cannot calculate measured spread cost");return;}spreadCost=MathAbs(spreadPnl);
 Audit("signal",buy?"confirmed M5 high-low-high break":"confirmed M5 low-high-low break",entry,sl,tp,lots,risk);
 bool sent=buy?trade.Buy(lots,_Symbol,0,sl,tp,"D14H1M5_RESEARCH"):trade.Sell(lots,_Symbol,0,sl,tp,"D14H1M5_RESEARCH");
 ...
 else {initialRiskDistance=MathAbs(trade.ResultPrice()-sl);managedTicket=0;Audit("accepted",(string)trade.ResultDeal(),trade.ResultPrice(),sl,tp,lots,risk);}
}
void ReadFive()
{
 if(zoneDir==0 || zoneDir!=bias)return;
 int n=MathMax(2*InpM5Pivot+1,InpHoldBars),k=n-1,p=k-InpM5Pivot;
 MqlRates b[];if(CopyRates(_Symbol,PERIOD_M5,1,n,b)!=n)return;
 MqlRates c=b[k];if(c.time<zoneCreated || c.time+300>TimeCurrent())return;
 if(InpMaxZoneHours>0 && TimeCurrent()-zoneCreated>InpMaxZoneHours*3600){ClearZone("zone expired");return;}
 bool buy=zoneDir>0;
 if((buy && c.close<zoneLow) || (!buy && c.close>zoneHigh)){ClearZone("M5 close through far edge");return;}
 if(attempted || OurPosition())return;
 if(InpDirection!=0 && zoneDir!=InpDirection)return;
 if(touchTime==0)
 {
 ...
 if(heldAt==0)
 {
  int begin=n-InpHoldBars;if(b[begin].time<touchTime)return;
  int wicks=0;for(int i=begin;i<n;i++){if(i>begin && b[i].time-b[i-1].time!=300)return;if(Rejection(b[i],zoneDir,zoneLow,zoneHigh))wicks++;}
  if(wicks<2)return;
  heldAt=c.time+300;holdExtreme=buy?b[begin].low:b[begin].high;
  for(int i=begin+1;i<n;i++)holdExtreme=buy?MathMin(holdExtreme,b[i].low):MathMax(holdExtreme,b[i].high);
  holds++;Audit("held","at least two rejection wicks in completed holding window");return;
 }
 // No retrospective swings from before the holding confirmation are accepted.
 if(b[p].time>=heldAt)
 {
  if(IsPivot(b,true,InpM5Pivot))mHigh=b[p];if(IsPivot(b,false,InpM5Pivot))mLow=b[p];
 }
 MqlDateTime clock;TimeToStruct(TimeCurrent(),clock);if(clock.hour<InpSessionStart || clock.hour>=InpSessionEnd)return;
 if(buy && mHigh.time>=heldAt && mLow.time>mHigh.time && mLow.low>holdExtreme && c.close>mHigh.high && b[k-1].close<=mHigh.high)SendEntry(true);
 if(!buy && mLow.time>=heldAt && mHigh.time>mLow.time && mHigh.high<holdExtreme && c.close<mLow.low && b[k-1].close>=mLow.low)SendEntry(false);
}
void Manage()
{
 if(InpManagement==0 && InpMaxHoldHours==0)return;
 for(int i=PositionsTotal()-1;i>=0;i--)
 {
  ulong ticket=PositionGetTicket(i);if(ticket==0 || PositionGetString(POSITION_SYMBOL)!=_Symbol || PositionGetInteger(POSITION_MAGIC)!=InpMagic)continue;
  bool buy=PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_BUY;
  double entry=PositionGetDouble(POSITION_PRICE_OPEN),old=PositionGetDouble(POSITION_SL),tp=PositionGetDouble(POSITION_TP);
  if(managedTicket!=ticket){managedTicket=ticket;initialRiskDistance=MathAbs(entry-old);}
  if(InpMaxHoldHours>0 && TimeCurrent()-(datetime)PositionGetInteger(POSITION_TIME)>=InpMaxHoldHours*3600)
  {if(trade.PositionClose(ticket) && trade.ResultRetcode()==TRADE_RETCODE_DONE)Audit("time_exit","maximum holding hours");else Audit("manage_error",trade.ResultRetcodeDescription());return;}
  if(initialRiskDistance<=0 || InpManagement==0)return;
  MqlTick tick;if(!SymbolInfoTick(_Symbol,tick))return;
  double market=buy?tick.bid:tick.ask,gain=(buy?market-entry:entry-market)/initialRiskDistance,next=old;
  if(InpManagement==1 && gain>=1)next=entry;
  if(InpManagement==2 && gain>=1.5)next=buy?market-initialRiskDistance:market+initialRiskDistance;
  if(InpManagement==3 && gain>=1)
  {
   MqlRates b[];int n=2*InpM5Pivot+1;
   if(CopyRates(_Symbol,PERIOD_M5,1,n,b)==n && IsPivot(b,!buy,InpM5Pivot))next=buy?b[InpM5Pivot].low-Step():b[InpM5Pivot].high+Step();
  }
  next=RoundPrice(next,!buy);double minDist=SymbolInfoInteger(_Symbol,SYMBOL_TRADE_STOPS_LEVEL)*_Point;
  if((buy && next>old+Step()/2 && market-next>MathMax(minDist,Step())) || (!buy && next<old-Step()/2 && next-market>MathMax(minDist,Step())))
  {if(trade.PositionModify(ticket,next,tp) && trade.ResultRetcode()==TRADE_RETCODE_DONE)Audit("trail","ratchet only",entry,next,tp);else Audit("manage_error",trade.ResultRetcodeDescription());}
 }
}
void OnTick()
{
 ...
 Manage();ReadDaily();ReadHour();ReadFive();
}
...
int OnInit()
{
 ...
 if(InpH1Pivot<1 || InpH1Pivot>4 || InpM5Pivot<1 || InpM5Pivot>4 || InpHoldBars<2 || InpHoldBars>8 || InpTargetR<=0 || InpStopBufferATR<0)return INIT_PARAMETERS_INCORRECT;
 ...
 audit=FileOpen("D14Optimized-"+(string)InpMagic+".csv",FILE_WRITE|FILE_CSV|FILE_ANSI,',');if(audit==INVALID_HANDLE)return INIT_FAILED;
 FileWrite(audit,"server_time","event","detail","bias","zone_dir","zone_origin","zone_created","zone_low","zone_high","touch_time","held_at","hold_extreme","m5_high_time","m5_high","m5_low_time","m5_low","entry","stop","target","lots","risk_cash","equity","entry_spread_cost");
...
void OnDeinit(const int reason)
{
 ...
 if(InpExportBars){ExportBars(PERIOD_D1);ExportBars(PERIOD_H1);ExportBars(PERIOD_M5);}
