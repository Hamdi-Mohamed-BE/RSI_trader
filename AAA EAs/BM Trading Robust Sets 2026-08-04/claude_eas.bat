...
title BM Trading - Claude EAs (Best Recommended 2026-09-01 + Nasdaq 5M DI filter ON)

rem claude_eas.bat (2026-09-25): identical to BEST RECOMMENDED 2026-09-01 except Nasdaq 5M Candle Momentum,
rem which uses the same EA with the +DI/-DI agreement filter (Nasdaq 5M DI Promotion 2026-09-25).
echo Applying Claude EAs: the Best Recommended 2026-09-01 portfolio with one change...
echo - CHANGED: Nasdaq 5M Candle Momentum requires +DI/-DI agreement with the 09:30 New York M5 signal (DI 14, ON by default)
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0_Auto Deploy\Start-Dynamic-Portfolio.ps1" -SafetyMode STANDARD -UseRecommendedSelections -UseClaudeSelections %*
set "BM_EXIT=%ERRORLEVEL%"

echo.
if not "%BM_EXIT%"=="0" (
  echo The Claude EAs installer stopped without starting the portfolio.
) else (
  echo Claude EAs installation finished.
)
