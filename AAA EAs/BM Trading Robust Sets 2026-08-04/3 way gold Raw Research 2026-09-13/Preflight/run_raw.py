...
def analyze(period,engine,runid,delay,report,journal):
    ...
    for pid,g in groups.items():
        ...
        matching=[time for time in signals if stamp(time).replace(minute=0,second=0)==stamp(decision_time)]
        assert len(matching)==1 and signals[matching[0]][0][eng-1]==direction,(tag,'entry signal mismatch',e)
        assert stamp(matching[0])<=stamp(e['time'])
...
def run(period,engine=0,delay=1):
...
if __name__=='__main__':
    cli=argparse.ArgumentParser();cli.add_argument('--periods',default=','.join(WINDOWS));cli.add_argument('--engine',type=int,default=0,choices=range(4));cli.add_argument('--delay',type=int,default=1);cli.add_argument('--compile-only',action='store_true');cli.add_argument('--components',action='store_true');a=cli.parse_args()
    prepare()
    if not a.compile_only:
        for period in a.periods.split(','):run(period,a.engine,a.delay)
        if a.components:
