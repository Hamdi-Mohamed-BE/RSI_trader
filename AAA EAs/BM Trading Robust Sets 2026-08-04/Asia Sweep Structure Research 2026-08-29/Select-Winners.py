...
def main() -> None:
    ...
    pattern = re.compile(rf"^({symbols})--(.+)--development\.htm$", re.I)
    rows: list[dict] = []
    for path in args.reports.glob("*.htm"):
        ...
        sample_penalty = max(0, 30 - row["trades"]) * 0.30
        row["score"] = row["return_pct"] + 9.0 * (row["pf"] - 1.0) - 0.55 * row["dd"] - sample_penalty
        if row["trades"] < 12 or row["pf"] <= 0.0:
    ...
    for symbol in SYMBOLS:
        ...
        if not candidates:
            raise RuntimeError(f"No development reports for {symbol}")
