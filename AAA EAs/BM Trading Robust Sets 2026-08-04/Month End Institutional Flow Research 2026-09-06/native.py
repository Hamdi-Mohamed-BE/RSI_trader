"""Compile and natively validate the two frozen month-end configurations."""
...
SOURCE = ROOT / "EA" / "Calyx Month End Flow EA.mq5"
EXPERT_DIR = "Calyx Month End Flow"
SYMBOLS = ("USTEC", "US30")
TF = {"M5": 5, "M15": 15, "M30": 30, "H1": 16385}
WINDOW = {"classic": 0, "last1": 1, "last2": 2, "last3": 3, "first1": 4, "first3": 5, "last2-first2": 6, "last3-first3": 7}
ENTRY = {"london-open": 0, "ny-open": 1, "ny-first-hour": 2, "ny-power-hour": 3}
CONFIRM = {"none": 0, "candle": 1, "breakout": 2}
TREND = {"none": 0, "ema20": 1, "ema50": 2, "momentum20": 3, "ema20-momentum20": 4, "prior-day-reversal": 5, "prior-day-strength": 6}
DIRECTION = {"long-only": 0, "short-only": 1, "daily-trend": 2}
STOP = {"atr": 0, "prior-day": 1, "signal-candle": 2}
...
def inputs(config, symbol):
    return dict(
        InpSignalTimeframe=TF[config["timeframe"]], InpCalendarWindow=WINDOW[config["window"]], InpEntryTime=ENTRY[config["entry_time"]],
        InpConfirmation=CONFIRM[config["confirmation"]], InpTrendFilter=TREND[config["trend"]], InpDirection=DIRECTION[config["direction"]],
        InpMaximumHoldHours=config["max_hold_hours"], InpStopMode=STOP[config["stop_mode"]], InpStopValue=config["stop_value"],
        InpRewardRisk=config["rr"], InpManagement=MANAGEMENT[config["management"]], InpRiskPercent=1.0,
        InpMaximumSpreadRiskPercent=20.0, InpMaximumDeviationPoints=100, InpMagic=969060500 + SYMBOLS.index(symbol), InpTesterOnly=True,
        InpTesterServerUTCOffsetHours=0, InpUseAutomaticLiveServerOffset=True, InpManualLiveServerUTCOffsetHours=0,
    )
...
def run(symbol, config, stage, model, timeout=1200):
    ...
    report_dir = TESTER / "reports" / "calyx-month-end-flow"; report_dir.mkdir(parents=True, exist_ok=True); report = report_dir / (case + ".htm")
    if report.exists(): report.rename(report.with_name(case + f".old-{time.time_ns()}.htm"))
    jobs = TESTER / "backtest-configs" / "calyx-month-end-flow"; jobs.mkdir(parents=True, exist_ok=True); ini = jobs / (case + ".ini")
...
Enabled=0
[Tester]
Expert={EXPERT_DIR}\\Calyx Month End Flow EA
...
ToDate={end}
ForwardMode=0
Report=reports\\calyx-month-end-flow\\{case}.htm
ReplaceReport=1
ShutdownTerminal=1
UseCloud=0
Visual=0
""", encoding="utf-8-sig")
    ...
    if len(trades) != row["trades"] or abs(sum(item["net"] for item in trades) - row["net_profit"]) > .06: raise RuntimeError(f"{case}: ledger mismatch")
    dump(output / "trades.json", trades); row.update(symbol=symbol, stage=stage, model=model, config=config, inputs=params, fingerprint=fingerprint, elapsed_seconds=round(time.monotonic()-began, 2), audit=dict(trades=len(trades)))
