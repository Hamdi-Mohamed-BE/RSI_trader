...
SYMBOLS = tuple(item.strip().lower() for item in os.environ.get("ELLIOTT_SYMBOLS", "xauusd,xagusd,ustec,us30,btcusd,gbpjpy,eurusd").split(",") if item.strip())
...
def select_phase(args: argparse.Namespace) -> None:
    ...
    screen_cases = int(os.environ.get("ELLIOTT_SCREEN_CASES_PER_SYMBOL", "12"))
    expected = len(SYMBOLS) * {"screen": screen_cases, "stoprr": 21, "trailing": 6, "session": 5}[args.phase]
    ...
    lines = [f"# Elliott Wave 1-2-3 {args.phase} selection", "", "Development period: 2023-09-01 to 2025-08-31. This period alone selected the settings.", "", "| Symbol | TF | Variant | Return | PF | Win rate | Max DD | Trades | Sharpe | Recovery |", "|---|---:|---|---:|---:|---:|---:|---:|---:|---:|"]
    for symbol in SYMBOLS:
        row = winners[symbol]
...
def plot_candidates(rows: list[dict], winners: dict, phase: str, output: Path) -> None:
    ...
    fig.suptitle(f"Elliott Wave 1-2-3 — {phase} development comparison", fontsize=17, fontweight="bold")
...
def final_audit(args: argparse.Namespace) -> None:
    ...
    fig.suptitle("Elliott Wave 1-2-3 — untouched last-year validation", fontsize=17, fontweight="bold")
    ...
    lines = ["# Elliott Wave 1-2-3 final native-MT5 audit", "", "The two-year development sample selected every parameter. The last year was then run once, untouched.", "", "## Untouched last-year comparison", "", "| Symbol | Config | Return | PF | Win rate | Max DD | Trades | Sharpe | Recovery |", "|---|---|---:|---:|---:|---:|---:|---:|---:|"]
    for symbol in SYMBOLS:
