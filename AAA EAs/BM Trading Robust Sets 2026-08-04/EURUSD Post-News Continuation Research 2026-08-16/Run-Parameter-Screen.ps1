...
$configRoot=Join-Path $testerRoot 'backtest-configs\eurusd-post-news-20260816\screen'
$reportRoot=Join-Path $testerRoot 'reports\eurusd-post-news-20260816\screen'
...
foreach($delay in @(3,5)){
    foreach($body in @(0.35,0.50)){
        foreach($retrace in @(0.40,0.50,0.60)){
            $slug=('d{0}-b{1}-r{2}' -f $delay,[int]($body*100),[int]($retrace*100))
            $cases.Add([pscustomobject]@{slug=$slug;delay=$delay;body=$body;retrace=$retrace;rr=1.8})
        }
    }
}
...
foreach($case in $cases){
    ...
    $setName='PNC SCREEN '+$case.slug+'.set'
    ...
    $relativeReport='reports\eurusd-post-news-20260816\screen\'+$case.slug+'.htm'
    $config=@"
[Common]
Login=472334559
