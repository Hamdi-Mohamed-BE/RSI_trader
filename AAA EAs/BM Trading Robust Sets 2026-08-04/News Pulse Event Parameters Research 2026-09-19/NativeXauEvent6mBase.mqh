...
#include "C:/Users/hama101/Desktop/geek/ai trader/AAA EAs/BM Trading Robust Sets 2026-08-04/News Pulse Event Parameters Research 2026-09-19/Deployment/NewsPulseTesterCalendar.mqh"
...
input bool   InpUseXauEventSpecific=true;       // XAU/GOLD only; NFP/CPI/FOMC promoted 2026-09-19
...
bool NP_XauEventSpecific()
{
   string symbol=_Symbol; StringToUpper(symbol);
   return InpUseXauEventSpecific && (StringFind(symbol,"XAU")==0 || StringFind(symbol,"GOLD")==0);
}
int NP_LeadSeconds(const string kind)
{
   if(!NP_XauEventSpecific()) return InpPlacementLeadSeconds;
   if(kind=="NFP") return 10;
   if(kind=="CPI") return 5;
   if(kind=="FOMC") return 60;
   return InpPlacementLeadSeconds;
}
int g_np_lead=60,g_np_hold=120;
double g_np_offset=12,g_np_stop=10,g_np_trail_start=3,g_np_trail_distance=10,g_np_tp=0;
bool g_np_closed_m1=false;
void NP_ApplyEventParameters(const string kind)
{
   ...
   g_np_tp=0; g_np_closed_m1=false;
   if(!NP_XauEventSpecific()) return;
   if(kind=="NFP") {g_np_lead=10;g_np_offset=2;g_np_stop=2;g_np_tp=0;g_np_trail_start=0;g_np_trail_distance=4;g_np_hold=60;g_np_closed_m1=true;}
   if(kind=="CPI") {g_np_lead=5;g_np_offset=1;g_np_stop=2;g_np_tp=0;g_np_trail_start=1;g_np_trail_distance=10;g_np_hold=300;g_np_closed_m1=true;}
   if(kind=="FOMC") {g_np_lead=60;g_np_offset=1;g_np_stop=2;g_np_tp=5.5;g_np_trail_start=.5;g_np_trail_distance=4;g_np_hold=120;}
}
...
bool NP_SendStraddle(const datetime event_time,const long event_id,const string kind)
{
   ...
   if(g_np_closed_m1)
   {
      double h=iHigh(_Symbol,PERIOD_M1,1),l=iLow(_Symbol,PERIOD_M1,1);
      if(h<=0 || l<=0 || h<l) {Print("News Pulse: closed M1 anchor unavailable; waiting for history.");return false;}
