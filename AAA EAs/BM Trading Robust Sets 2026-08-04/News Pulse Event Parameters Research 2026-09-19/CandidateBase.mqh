...
#include "..\AAA Final EAs\AAA Final News Pulse EA\AAA_Final_Common.mqh"
#include "..\AAA Final EAs\AAA Final News Pulse EA\SafeRegimeFilter.mqh"
#include "..\AAA Final EAs\AAA Final News Pulse EA\DynamicTrailingSessionFilter.mqh"
...
int InpPlacementLeadSeconds=60;       // place the straddle this many seconds before release
...
double InpEntryOffsetPrice=12.0;         // buy above Ask / sell below Bid
double InpStopLossPrice=10.0;            // one R
double InpTrailStartR=3.0;               // start trailing after +3R
double InpTrailDistancePrice=10.0;        // trail one R behind current price
int InpForceCloseSecondsAfterEvent=120;
...
bool NP_SendStraddle(const datetime event_time,const long event_id,const string kind)
{
   ...
   double buy_entry=AAA_Price(_Symbol,NPX_BuyAnchor(tick)+InpEntryOffsetPrice);
   double sell_entry=AAA_Price(_Symbol,NPX_SellAnchor(tick)-InpEntryOffsetPrice);
   ...
   if(allow_buy)
   {
      buy_ok=AAA_Trade.BuyStop(buy_lots,buy_entry,_Symbol,buy_sl,(NPX_TPR>0 ? AAA_Price(_Symbol,buy_entry+NPX_TPR*InpStopLossPrice):0.0),ORDER_TIME_SPECIFIED,expiry,prefix+"B");
      if(!buy_ok)
         Print("News Pulse: buy-stop placement failed: ",AAA_Trade.ResultRetcodeDescription());
   }
   if(allow_sell)
   {
      sell_ok=AAA_Trade.SellStop(sell_lots,sell_entry,_Symbol,sell_sl,(NPX_TPR>0 ? AAA_Price(_Symbol,sell_entry-NPX_TPR*InpStopLossPrice):0.0),ORDER_TIME_SPECIFIED,expiry,prefix+"S");
      if(!sell_ok)
