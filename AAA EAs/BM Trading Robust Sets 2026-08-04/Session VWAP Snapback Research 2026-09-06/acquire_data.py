"""Read-only M5 history acquisition from the isolated Exness demo research terminal."""
...
def main():
    ...
    try:
        ...
        if not account or account.trade_mode != mt5.ACCOUNT_TRADE_MODE_DEMO:
            raise RuntimeError('Historical acquisition is restricted to the isolated demo research terminal')
        meta = {'server': account.server, 'currency': account.currency, 'acquired_utc': datetime.now(timezone.utc).isoformat(), 'timeframe': 'M5', 'symbols': {}}
        for symbol in SYMBOLS:
            ...
            for year in range(START.year, END.year + 1):
                for month in (1,4,7,10):
                    ...
                    chunk = None
                    for _ in range(10):
                        chunk = mt5.copy_rates_range(symbol, mt5.TIMEFRAME_M5, begin, finish)
                        if chunk is not None and len(chunk): break
                        time.sleep(2)
                    if chunk is None or not len(chunk):
                        raise RuntimeError(f'{symbol}: no M5 data for {begin.date()}..{finish.date()} ({mt5.last_error()})')
