...
enum ENUM_BROKER_SESSION_PROFILE
{
   BROKER_SESSION_ORIGINAL_NY_CLOSE=0,
   BROKER_SESSION_EXNESS_INDEX=1,
   BROKER_SESSION_EXNESS_STOCK=2
};
...
input ENUM_BROKER_SESSION_PROFILE InpBrokerSessionProfile=BROKER_SESSION_ORIGINAL_NY_CLOSE;
...
// Exness keeps relevant CFD trading cut-offs on a fixed UTC schedule. This
// maps those cut-offs back to New York time so an entry is sent before the
// broker changes the symbol to close-only/closed mode.
int EffectiveCashCloseMinute(const MqlDateTime &ny)
{
   if(InpBrokerSessionProfile==BROKER_SESSION_EXNESS_INDEX)
      return (NewYorkDateUsesDST(ny) ? 16*60 : 15*60+59);
   if(InpBrokerSessionProfile==BROKER_SESSION_EXNESS_STOCK)
      return (NewYorkDateUsesDST(ny) ? 15*60+44 : 14*60+44);
   return InpCashCloseHour*60+InpCashCloseMinute;
}
...
bool PreviousCashClose(const MqlDateTime &today_ny,const datetime current_open_server,double &previous_close)
{
   ...
   for(int i=count-1;i>=0;i--)
   {
      ...
      int target_minute=EffectiveCashCloseMinute(bar_ny)-1;
...
bool NegativeDayReturn(const MqlDateTime &today_ny,double &day_return_percent,int &bar_count)
{
   ...
   int effective_close=EffectiveCashCloseMinute(today_ny);
   finish.hour=effective_close/60; finish.min=effective_close%60; finish.sec=0;
   datetime from=NewYorkToServer(start);
...
void TryEntry(const MqlDateTime &now_ny)
{
   ...
   int minute_of_day=now_ny.hour*60+now_ny.min;
   int close_minute=EffectiveCashCloseMinute(now_ny);
