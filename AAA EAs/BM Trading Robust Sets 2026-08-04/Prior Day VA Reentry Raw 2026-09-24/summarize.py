"""Summarise the previous-day value area re-entry raw runs (XAUUSD).
...
def main():
    out = {}
    for run in sorted((ROOT / "native").glob("pdva-*-5y-m4-d150/run.json")):
        meta = json.loads(run.read_text(encoding="utf-8"))
        s, tr = parse(run.parent / Path(meta["report"]).name)
        row = dict(journal_flags=meta["journal_flags"], ea_summary=meta["ea_summary"],
