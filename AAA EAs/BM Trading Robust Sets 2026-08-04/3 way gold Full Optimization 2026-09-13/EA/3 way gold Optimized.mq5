...
#property description "3 way gold parameter research. Tester only. Defaults preserve raw v0.1."
...
void Manage()
{
 ...
 for(int i=PositionsTotal()-1;i>=0;i--)
 {
  ...
  if(risk<=0 || dir*(b[1].close-entry)<InpTriggerR*risk)continue;
  double next=oldSL;
  if(InpManagement==1 || InpManagement==3)next=dir>0?MathMax(next,entry):MathMin(next,entry);
  if(InpManagement==2 || InpManagement==3)
  {
   double candidate=b[1].close-dir*InpTrailATR*atr[1];
...
{
 if(!dir || (InpEngine!=0 && InpEngine!=engine) || (InpDirection!=0 && dir!=InpDirection) || EngineWeight(engine)<=0)return;
 ...
 double distance=MathMax(EngineStop(engine)*atr[1],minDist+(quote.ask-quote.bid));
...
int OnInit()
{
 ...
 prefix="3WGO-"+(string)InpAuditRun+"-"+(string)InpEngine;
...
void OnTick()
{
 ...
 int s3=BreakoutSignal(b[1].close,hi,lo,tr,atr[1],atr[2],InpExpansionATR,InpRisingATR);
