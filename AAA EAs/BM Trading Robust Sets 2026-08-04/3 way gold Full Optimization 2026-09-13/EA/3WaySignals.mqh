// Parameterized definitions; defaults preserve raw v0.1. Every argument comes from CLOSED H1 candles.
int MomentumSignal(double c1,double c2,double e20_1,double e20_2,double trend50,double trend200,double strength,double plus,double minus,double adxMin=25.0)
{
 if(strength<adxMin)return 0;
...
int ChangeSignal(double f1,double f2,double s1,double s2,double oscillator,double threshold=50.0)
{
 if(f2<=s2 && f1>s1 && oscillator>threshold)return 1;
 if(f2>=s2 && f1<s1 && oscillator<100.0-threshold)return -1;
 return 0;
}
int BreakoutSignal(double c,double hi20,double lo20,double tr,double atr1,double atr2,double expansion=1.5,bool requireRising=true)
{
 if(atr2<=0 || tr<expansion*atr2 || (requireRising && atr1<=atr2))return 0;
 if(c>hi20)return 1;
 if(c<lo20)return -1;
 return 0;
}
