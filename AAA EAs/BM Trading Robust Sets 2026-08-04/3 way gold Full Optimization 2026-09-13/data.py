...
def load():
    cache();d=np.load(ROOT/'Data'/'prices.npz');h=d['h1'];m=d['m1']
...
def verify_raw(d):
    ...
    save(ROOT/'Data'/'independent-verification.json',dict(decisions=len(rows),signal_exact=True,max_indicator_difference=checks,sampled_m1_h1_checks=len(z)))
