...
EXPERT=Path('AAA Research')/'3 way gold Independent 20260913'
NAME='3 way gold Independent'
from params import DEFAULT,normalize,ident,RAW,effective,OPT
WINDOWS={}
...
def prepare():
    for p in (ROOT/'Runs',ROOT/'Sets',ROOT/'Audit',ROOT/'Backtest Reports',TESTER/'MQL5'/'Experts'/EXPERT,
              TESTER/'MQL5'/'Profiles'/'Tester',TESTER/'backtest-configs'/'3-way-gold-ind',TESTER/'reports'/'3-way-gold-ind'):
...
def validate_decisions(rows,bars):
    ...
    for row in rows:
        ...
        if feature_checks==0:
            unfiltered=np.zeros((len(times),3),dtype=np.int8)
            for eng in (1,2,3):
                v=effective(c,eng);v['InpEngine']=0;v['InpDirection']=0
                ss,_=signals(DATA,v);unfiltered[:,eng-1]=ss[:,eng-1]
        ...
        for eng,col in [(1,'momentum_atr'),(2,'change_atr'),(3,'breakout_atr')]:
            p=effective(CURRENT,eng)['InpATRPeriod']
            assert abs(x[col]-DATA['ind'][p][0][i])<.002
...
def run(config,window,stage='native',delay=1,model=4,force=False):
    ...
    content+=f'FromDate={start}\nToDate={end}\nForwardMode=0\nReport=reports\\3-way-gold-ind\\{tag}.htm\nReplaceReport=1\nShutdownTerminal=1\nUseLocal=1\nUseRemote=0\nUseCloud=0\nVisual=0\n'
    config=TESTER/'backtest-configs'/'3-way-gold-ind'/(tag+'.ini');config.write_text(content,encoding='utf-16')
    ...
    report=TESTER/'reports'/'3-way-gold-ind'/(tag+'.htm')
