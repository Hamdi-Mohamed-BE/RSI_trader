...
PHASE_COUNTS = {"screen": 7, "stop": 7, "rr": 8, "trailing": 6, "session": 5}
...
def plot_candidates(rows: list[dict], winners: dict, phase: str, output: Path) -> None:
    ...
    axes = list(grid.flat)
    fig.suptitle(f"POC + Fibonacci Volume Profile — {phase} development comparison", fontsize=18, fontweight="bold")
...
def select_phase(args: argparse.Namespace) -> None:
    ...
    lines = [
        f"# POC + Fibonacci Volume Profile — {args.phase} selection",
        "",
...
def monte_carlo(outcomes: list[float], initial: float, samples: int = 10_000) -> dict:
    ...
    if not outcomes:
        return empty
    rng = random.Random(940410 + len(outcomes))
    returns: list[float] = []
...
def final_audit(args: argparse.Namespace) -> None:
    ...
    result={"test_design":{"development":"2023-09-01 to 2025-08-31","locked":"2025-09-01 to 2026-09-01","full":"2023-09-01 to 2026-09-01","development_model":"MT5 1-minute OHLC for parameter search","final_model":"MT5 Every Tick with broker spread, commission, swap and random delay","risk_per_trade_pct":1.0,"monte_carlo":"10,000 five-trade block-bootstrap paths"},"development_profile_fibonacci_screen":selection_payloads["screen"]["rows"],"symbols":{}}

    equity_fig,equity_grid=plt.subplots(3,2,figsize=(16,15),constrained_layout=True)
    equity_axes=list(equity_grid.flat)
    equity_fig.suptitle("POC + Fibonacci Volume Profile — untouched last-year equity",fontsize=18,fontweight="bold")
    mc_fig,mc_grid=plt.subplots(3,2,figsize=(16,15),constrained_layout=True)
    mc_axes=list(mc_grid.flat)
    mc_fig.suptitle("POC + Fibonacci Volume Profile — 10,000-path block-bootstrap Monte Carlo",fontsize=18,fontweight="bold")
    ...
    lines += ["","## Selected configuration","","| Symbol | Profile/Fibonacci screen | Stop | RR | Trailing | Session |","|---|---|---|---|---|---|"]
    for symbol in SYMBOLS:
