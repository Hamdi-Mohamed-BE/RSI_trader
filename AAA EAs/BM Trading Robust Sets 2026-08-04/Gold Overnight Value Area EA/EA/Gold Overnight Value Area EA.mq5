...
// Raw strategy promoted 2026-09-19. Signal/exit defaults are unchanged.
#include "..\\..\\_Shared\\CalyxAdaptivePortfolio.mqh"
input bool InpEnableTrading=true;
input bool InpAdaptivePortfolioControls=false;
input int InpRiskMode=0; // 0=equity percent, 1=fixed account-currency amount
input double InpFixedRiskMoney=100;
input bool InpAutoServerUTCOffset=true; // manual offset is mandatory in tester
input bool InpWriteAudit=false;
string consumed_key="";
bool restore_day=true;
int ServerOffsetSeconds() {
 if(MQLInfoInteger(MQL_TESTER) || !InpAutoServerUTCOffset)return InpServerUTCOffsetHours*3600;
 return (int)MathRound((double)(TimeTradeServer()-TimeGMT())/1800.0)*1800;
}
string DayStateKey() {
 return "CALYX_GVA_"+IntegerToString(AccountInfoInteger(ACCOUNT_LOGIN))+"_"+_Symbol+"_"+IntegerToString(InpMagic);
}
bool ConsumedToday(datetime today) {
 if(!MQLInfoInteger(MQL_TESTER) && GlobalVariableCheck(consumed_key) && (int)GlobalVariableGet(consumed_key)==daykey)return true;
 if(!HistorySelect(FromNY(today),TimeCurrent()))return true; // fail closed if history unavailable
 for(int i=HistoryDealsTotal()-1;i>=0;i--) {
  ulong id=HistoryDealGetTicket(i);
  if(HistoryDealGetString(id,DEAL_SYMBOL)!=_Symbol || HistoryDealGetInteger(id,DEAL_MAGIC)!=InpMagic)continue;
  long e=HistoryDealGetInteger(id,DEAL_ENTRY);
  if(e==DEAL_ENTRY_IN || e==DEAL_ENTRY_INOUT)return true;
 }
 return false;
}
bool ConsumeDay() {
 if(MQLInfoInteger(MQL_TESTER))return true;
 if(!GlobalVariableCheck(consumed_key))GlobalVariableSet(consumed_key,0);
 double before=GlobalVariableGet(consumed_key);
 if((int)before==daykey)return false;
 bool ok=GlobalVariableSetOnCondition(consumed_key,daykey,before);
...
datetime ToNY(datetime server) {datetime u=server-ServerOffsetSeconds();return u+NYOffset(u)*3600;}
datetime FromNY(datetime wall) {
 // Session boundaries are 18:00/09:30/16:00, never the ambiguous DST hour.
 datetime u=wall+5*3600;return wall-NYOffset(u)*3600+ServerOffsetSeconds();
}
...
void OnTick() {
 ...
 if(Key(ny)!=daykey) {
  daykey=Key(ny);consumed=restore_day?ConsumedToday(today):false;restore_day=false;ready=false;attempted=false;bar_seen=0;cutoff=Cutoff(today);
 ...
 if(!ready || consumed || !InpEnableTrading)return;
 ulong id;if(OwnPosition(id))return;
 ...
 if(side==0)return;
 consumed=true;if(!ConsumeDay())return;signals++;
 ...
 if(multiplier<=0){Audit("adaptive_block",now);return;}
 double desired=(InpRiskMode==1?InpFixedRiskMoney:equity*InpRiskPercent/100)*multiplier;
...
int OnInit() {
 if(InpExpectedLogin>0 && AccountInfoInteger(ACCOUNT_LOGIN)!=InpExpectedLogin)return INIT_FAILED;
 if(InpExpectedServer!="" && AccountInfoString(ACCOUNT_SERVER)!=InpExpectedServer)return INIT_FAILED;
 string sym=_Symbol;StringToUpper(sym);
 if(StringFind(sym,"XAU")<0 && StringFind(sym,"GOLD")<0){Print("Gold Overnight VA requires a gold symbol.");return INIT_PARAMETERS_INCORRECT;}
 if(!MQLInfoInteger(MQL_TESTER) && AccountInfoInteger(ACCOUNT_MARGIN_MODE)!=ACCOUNT_MARGIN_MODE_RETAIL_HEDGING){Print("Independent portfolio positions require a hedging account.");return INIT_FAILED;}
 if(InpValueAreaPercent<=0 || InpValueAreaPercent>100 || InpBins>1000 || InpMinimumProfileBars<1 || InpRiskMode<0 || InpRiskMode>1 || InpFixedRiskMoney<=0)return INIT_PARAMETERS_INCORRECT;
 // Production is the approved raw rule set; candidate parameters are not deployed.
 if(InpDirectionMode!=0 || InpBins!=64 || InpValueAreaPercent!=70 || InpStopMode!=0 || InpMinimumR!=0 || InpTargetR!=0 || InpBreakEvenR!=0 || InpTrailDistanceR!=0 || InpEntryEndMinute!=960 || InpExitMinute!=960)return INIT_PARAMETERS_INCORRECT;
 consumed_key=DayStateKey();
 ...
 if(InpWriteAudit) {
  audit_file=FileOpen(InpCase+"-audit.csv",FILE_WRITE|FILE_CSV|FILE_ANSI,',');
 ...
 if(!MQLInfoInteger(MQL_TESTER))EventSetTimer(10);
...
// Timer only manages exits; it never generates entries without a market tick.
void OnTimer() {
 datetime now=MQLInfoInteger(MQL_TESTER)?TimeCurrent():TimeTradeServer();
 if(now<=0)return;
 cutoff=Cutoff(Midnight(ToNY(now)));
 Manage(now);
}
