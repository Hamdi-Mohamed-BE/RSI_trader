...
input double             InpHeavyZoneVolumeFraction = 0.65;
...
input int                InpRetestSignalBars       = 5;
input bool               InpUseProfileBarrierTarget = false;
input double             InpMinimumBarrierRR       = 1.00;
                               ...
                               ProfileLevels &profile)
{
   ...
   double heavy_threshold = max_vol * ClampDouble(InpHeavyZoneVolumeFraction, 0.10, 1.00);
...
bool BuildSignal(TradeSignal &signal)
{
   ...
   for(int d = 0; d < 2; d++)
   {
      ...
      if(InpUseTranscriptPOCMode && InpUseProfileBarrierTarget)
      {
         double barrier = NearestProfileBarrier(dir, entry);
         double barrier_buffer = MathMax(buffer, MinimumStopDistance());
         double barrier_tp = (dir > 0 ? barrier - barrier_buffer : barrier + barrier_buffer);
         double barrier_reward = MathAbs(barrier_tp - entry);
         if(barrier > 0.0 &&
            ((dir > 0 && barrier_tp > entry) || (dir < 0 && barrier_tp < entry)) &&
            barrier_reward / risk_distance >= InpMinimumBarrierRR &&
            barrier_reward < MathAbs(tp - entry))
            tp = barrier_tp;
      }
...
bool FindCandidateLevel(const int dir, CandidateLevel &candidate)
{
   ...
   if(InpUseTranscriptPOCMode)
      return FindTranscriptPOCLevel(dir, g_prev_day_profile, buffer, candidate);
...
bool FindTranscriptPOCLevel(const int dir,
                            const ProfileLevels &profile,
                            const double buffer,
                            CandidateLevel &candidate)
{
   if(!profile.valid || profile.poc <= 0.0)
      return false;

   double zone_low = (InpUseHeavyVolumeZoneEdge && profile.hvn_low > 0.0
                      ? profile.hvn_low : profile.poc - buffer);
   double zone_high = (InpUseHeavyVolumeZoneEdge && profile.hvn_high > 0.0
                       ? profile.hvn_high : profile.poc + buffer);
   ...
   datetime cutoff = iTime(_Symbol, InpExecutionTF, MaxInt(InpRetestSignalBars, 2));
   ...
   for(int i = 0; i < copied; i++)
   {
      ...
      if(dir > 0 && history[i].high >= zone_high + InpMinimumDepartureATR * atr)
         departed = true;
      if(dir < 0 && history[i].low <= zone_low - InpMinimumDepartureATR * atr)
         departed = true;

      bool touched = (history[i].low <= zone_high && history[i].high >= zone_low);
      if(InpRequireFirstRetest && was_departed && touched)
         return false;
   }
   if(!departed)
      return false;

   CandidateLevel probe;
   ResetCandidate(probe);
   probe.valid = true;
   probe.dir = dir;
   probe.name = "PD-HVN-FIRST";
   probe.low = NormalizePrice(zone_low);
   probe.high = NormalizePrice(zone_high);
   probe.price = NormalizePrice(dir > 0 ? zone_high : zone_low);
   probe.is_zone = true;

   MqlRates recent[];
   int recent_count = CopyRates(_Symbol, InpExecutionTF, 1, MaxInt(InpRetestSignalBars, 2), recent);
   if(recent_count <= 0)
      return false;
   ...
   {
      if(BarTouchesCandidate(dir, recent[i], probe))
      {
...
double NearestProfileBarrier(const int dir,const double entry)
{
   double best = (dir > 0 ? DBL_MAX : -DBL_MAX);
   ProfileLevels profiles[3];
   profiles[0] = g_prev_day_profile;
   profiles[1] = g_prev_week_profile;
   profiles[2] = g_swing_profile;
   for(int p = 0; p < 3; p++)
   {
      if(!profiles[p].valid)
         continue;
      double levels[5] = { profiles[p].poc, profiles[p].vah, profiles[p].val,
                           profiles[p].hvn_low, profiles[p].hvn_high };
      for(int i = 0; i < 5; i++)
      {
         double level = levels[i];
         if(dir > 0 && level > entry && level < best)
            best = level;
         if(dir < 0 && level < entry && level > best)
            best = level;
      }
   }
   if((dir > 0 && best == DBL_MAX) || (dir < 0 && best == -DBL_MAX))
      return 0.0;
   return best;
}
