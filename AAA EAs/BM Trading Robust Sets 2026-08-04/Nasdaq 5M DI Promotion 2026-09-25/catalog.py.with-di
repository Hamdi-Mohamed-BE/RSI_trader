        ...
        items.append(
            {
                ...
                "claude_expert_source": _extract_string(block, "ClaudeExpertSource") or None,
                "claude_set_source": _extract_string(block, "ClaudeSetSource") or None,
            }
        )
    if not items:
...
def get_catalog() -> list[Product]:
    ...
    for item in parse_installer_items():
        ...
        ) or bool(item["label"] == "Nasdaq 5M Candle Momentum" and item["claude_set_source"] and nasdaq_di is not None)
        is_nasdaq_di = item["label"] == "Nasdaq 5M Candle Momentum" and bool(item["claude_set_source"] and item["claude_expert_source"])
        limitations = [
