...
Adapted from Nasdaq 5M DI Filter Research 2026-09-23/Scripts/parse_native.py.
...
def main():
    out, ledgers = {}, {}
    for run in sorted((ROOT / "native").glob("us100orb-*/run.json")):
        meta = json.loads(run.read_text(encoding="utf-8"))
        rep = run.parent / Path(meta["report"]).name
        s, tr = parse(rep)
        s.update(name=meta["name"], variant=meta["variant"], filter=meta["filter"], production=meta["production"],
                 journal_flags=meta["journal_flags"], real_tick_lines=meta["real_tick_lines"],
                 variant_label=CONFIG["variants"][meta["variant"]]["label"],
                 filter_label=CONFIG["filters"][meta["filter"]]["label"])
        out[meta["name"]] = s
    ...
    if "H1-RR6-production" in ledgers and "H1-RR6-none" in ledgers:
        a, b = ledgers["H1-RR6-production"], ledgers["H1-RR6-none"]
        same = len(a) == len(b) and all(
            (x["entry"], x["exit"], round(x["net"], 2)) == (y["entry"], y["exit"], round(y["net"], 2)) for x, y in zip(a, b))
        print(f"PARITY production vs research(off): {len(a)} vs {len(b)} trades, identical={same}")
    for k, s in out.items():
        print(f"{k:<22} trades {s['trades']:>3}  PF {s['profit_factor']}  ret {s['return_pct']:>7}%  "
              f"win {s['win_rate_pct']:>5}%  eqDD {s['equity_dd_rel']}  quality {s['quality']}  late {s['exits_after_flat_or_overnight']}")
