...
bool PlaceSellStop(const double requested_entry,const MqlDateTime &now_ny)
{
   MqlTick tick;
   ...
   double stop=PriceToTick(entry+InpStopPips*PipSize(),1);
   double target=PriceToTick(entry-InpTargetPips*PipSize(),-1);
   ...
   trade.SetDeviationInPoints(InpMaxDeviationBrokerPoints);
   bool ok=trade.SellStop(lots,entry,_Symbol,stop,target,ORDER_TIME_SPECIFIED,expiry,"Sell Nasdaq 15min");
...
void EvaluateSetup()
{
   ...
   PlaceSellStop(setup.low-InpEntryBufferPips*PipSize(),now_ny);
}

int OnInit()
{
   if(InpOpeningRangeMinutes<=0 || InpEntryWindowMinutes<=0 || InpRiskPercent<=0.0 || InpRiskPercent>100.0 ||
      InpStopPips<=0.0 || InpTargetPips<=0.0 || InpEntryBufferPips<0.0 ||
