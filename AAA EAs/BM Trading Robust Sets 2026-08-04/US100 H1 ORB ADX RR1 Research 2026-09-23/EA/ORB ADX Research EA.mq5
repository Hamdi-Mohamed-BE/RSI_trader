...
enum ENUM_ORB_ADX_FILTER
{
   ORB_ADX_OFF=0,
   ORB_ADX_MINIMUM=1,
   ORB_ADX_DI_AGREES=2,
   ORB_ADX_DI_AND_MINIMUM=3
};
...
input group "ADX / DMI filter (research 2026-09-23)"
input ENUM_ORB_ADX_FILTER   InpAdxFilterMode=ORB_ADX_OFF;   // off = identical to v1.20 behaviour
input ENUM_TIMEFRAMES       InpAdxTimeframe=PERIOD_CURRENT;  // PERIOD_CURRENT = signal timeframe
...
// iADX buffers: 0=ADX, 1=+DI, 2=-DI. Shift 1 = last completed bar of the ADX timeframe,
// which is the just-closed breakout candle when the ADX timeframe equals the signal timeframe.
bool AdxAllows(const int direction)
{
   if(InpAdxFilterMode==ORB_ADX_OFF) return true;
   double adx[],plus_di[],minus_di[];
   if(g_adx_handle==INVALID_HANDLE || CopyBuffer(g_adx_handle,0,1,1,adx)!=1 ||
      CopyBuffer(g_adx_handle,1,1,1,plus_di)!=1 || CopyBuffer(g_adx_handle,2,1,1,minus_di)!=1)
      return false;
   bool strong=adx[0]>=InpAdxMinimum;
   bool agrees=(direction>0 ? plus_di[0]>minus_di[0] : minus_di[0]>plus_di[0]);
   if(InpAdxFilterMode==ORB_ADX_MINIMUM) return strong;
   if(InpAdxFilterMode==ORB_ADX_DI_AGREES) return agrees;
   return strong && agrees;
}
                             ...
                             const datetime range_end,const datetime trade_end)
{
   ...
   if(direction==0 || !DirectionAllows(direction) || !TrendAllows(direction) || !AdxAllows(direction) || !ProfileAllows(direction,bar.close)) return;
   if(InpRequireVWAP)
   {
...
void EvaluatePaperTickBreakout(const datetime range_start,const datetime trade_end)
{
   ...
   if(direction==0 || !DirectionAllows(direction) || !TrendAllows(direction) || !AdxAllows(direction)) return;
...
int OnInit()
{
   ...
   if(InpRiskPercent<=0.0 || InpRiskPercent>10.0 || InpOpeningRangeMinutes<5 ||
      ...
      InpAdxPeriod<2 || InpAdxMinimum<0.0)
      return INIT_PARAMETERS_INCORRECT;
   if(InpAdxFilterMode!=ORB_ADX_OFF)
   {
      ENUM_TIMEFRAMES adx_tf=(InpAdxTimeframe==PERIOD_CURRENT ? InpSignalTimeframe : InpAdxTimeframe);
      g_adx_handle=iADX(_Symbol,adx_tf,InpAdxPeriod);
