...
$expertFolder='AAA Research\LVN Volume Profile 20260904'
$expertName='LVN Volume Profile EA'
...
$configRoot=Join-Path $testerRoot ('backtest-configs\lvn-'+$Phase)
$testerReportRoot=Join-Path $testerRoot ('reports\lvn-'+$Phase)
...
function Default-Config {
    return [ordered]@{
        Entry=4;Days=10;Bins=64;LVNRatio=0.55;NeighborRatio=1.35;Tolerance=0.08;RetestBars=8;
        MinBody=0.20;VolumeBars=20;VolumeRatio=1.00;CloseLocation=0.65;
...
function Apply-Screen([System.Collections.IDictionary]$Config,[string]$Variant){
    switch($Variant){
        'raw5' {$Config.Entry=0;$Config.Days=5;$Config.MinBody=0.0;$Config.VolumeRatio=0.0;$Config.CloseLocation=0.50}
        'raw20' {$Config.Entry=0;$Config.Days=20;$Config.MinBody=0.0;$Config.VolumeRatio=0.0;$Config.CloseLocation=0.50}
        'breakout10' {$Config.Entry=1;$Config.Days=10}
        'bounce10' {$Config.Entry=2;$Config.Days=10}
        'retest10' {$Config.Entry=3;$Config.Days=10}
        'auto10' {$Config.Entry=4;$Config.Days=10}
...
function Write-Set([string]$Path,[System.Collections.IDictionary]$C,[long]$Magic){
    $text=@"
InpTimeframe=15
InpEntryMode=$($C.Entry)
InpATRPeriod=14
InpProfileLookbackDays=$($C.Days)
InpProfileBins=$($C.Bins)
InpMaximumLVNMeanRatio=$($C.LVNRatio)
InpMinimumNeighborRatio=$($C.NeighborRatio)
InpLevelToleranceATR=$($C.Tolerance)
InpRetestMaximumBars=$($C.RetestBars)
InpMinimumBodyATR=$($C.MinBody)
InpVolumeAverageBars=$($C.VolumeBars)
InpMinimumVolumeRatio=$($C.VolumeRatio)
...
function Run-Case([object]$SymbolCase,[string]$Variant,[System.Collections.IDictionary]$Config,[int]$Sequence,[string]$From,[string]$To,[int]$Model){
    ...
    $setName="LVN-$caseId.set"
    $savedSet=Join-Path $setsRoot $setName
    Write-Set $savedSet $Config (940409000+$Sequence)
    ...
    $reportRelative='reports\lvn-'+$Phase+'\'+$caseId+'.htm'
...
if($Phase -eq 'screen'){
    foreach($symbol in $symbols){foreach($variant in @('raw5','raw20','breakout10','bounce10','retest10','auto10')){
