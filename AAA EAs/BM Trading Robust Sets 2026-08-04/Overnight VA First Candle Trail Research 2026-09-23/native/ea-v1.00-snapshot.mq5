...
void Trail(datetime now) {
 ...
 datetime current=iTime(_Symbol,PERIOD_M5,0);if(current==trail_bar_seen)return;trail_bar_seen=current;
 MqlRates r[];if(CopyRates(_Symbol,PERIOD_M5,1,1,r)!=1)return;
 if(r[0].time<=signal_bar || r[0].time+300>now)return;   // only candles that closed after the signal candle
 if(!PositionSelectByTicket(id))return;
 bool buy=PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_BUY;
 double sl=PositionGetDouble(POSITION_SL),step=SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_SIZE);
 ...
 if(trade.PositionModify(id,candidate,0)){trail_moves++;}
...
void OnTick() {
 ...
 bool sent=side>0?trade.Buy(lots,_Symbol,0,stop,0,comment):trade.Sell(lots,_Symbol,0,stop,0,comment);
 if(sent && (trade.ResultRetcode()==TRADE_RETCODE_DONE || trade.ResultRetcode()==TRADE_RETCODE_DONE_PARTIAL)) {
  entries++;signal_bar=r[0].time;trail_bar_seen=iTime(_Symbol,PERIOD_M5,0);
...
int OnInit() {
 ...
 if(InpEntryMode<0 || InpEntryMode>1 || InpBins<2 || InpRiskPercent<=0)return INIT_PARAMETERS_INCORRECT;
...
void OnDeinit(const int reason) {
 ...
 PrintFormat("ONVT_SUMMARY|%s|days=%d|profiles=%d|missing=%d|first_inside=%d|signals=%d|entries=%d|skipped_open=%d|invalid_geometry=%d|rejected=%d|trail_moves=%d|trail_market_exits=%d|trail_rejects=%d|min_equity=%.2f|max_equity_dd=%.6f|balance=%.2f",
  InpCase,days,profiles,missing,inside_first,signals,entries,skipped_open,geometry,rejected,trail_moves,trail_market_exits,trail_rejects,min_equity,max_dd,AccountInfoDouble(ACCOUNT_BALANCE));
}
