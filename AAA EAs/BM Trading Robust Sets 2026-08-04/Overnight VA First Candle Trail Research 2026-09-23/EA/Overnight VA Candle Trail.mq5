...
// v1.20 regime filters (2026-09-24 revision; defaults reproduce v1.10 exactly):
// - InpTrendFilter=1: longs only if the last completed D1 close > D1 EMA(InpTrendEMA); shorts only if below.
// - InpMinDailyADX>0: trade only when the last completed D1 ADX(14) >= this value.
// A filtered signal consumes that day's attempt (one attempt per day, as before).
...
input int InpTrendFilter=0;           // 0 = off, 1 = trade only with the D1 EMA trend
input int InpTrendEMA=50;
input double InpMinDailyADX=0;        // 0 = off
...
int target_invalid=0,target_exits_seen=0,trend_filtered=0,adx_filtered=0;
int h_ema=INVALID_HANDLE,h_adx=INVALID_HANDLE;
...
void OnTick() {
 ...
 consumed=true;
 if(InpTrendFilter==1) {
  double e[];double c=iClose(_Symbol,PERIOD_D1,1);
  if(CopyBuffer(h_ema,0,1,1,e)!=1 || c<=0){trend_filtered++;return;}
  if(side>0 ? c<=e[0] : c>=e[0]){trend_filtered++;Audit("trend_filtered",now,side);return;}
 }
 if(InpMinDailyADX>0) {
  double a[];
  if(CopyBuffer(h_adx,0,1,1,a)!=1 || a[0]<InpMinDailyADX){adx_filtered++;Audit("adx_filtered",now,side);return;}
 }
 signals++;
...
int OnInit() {
 ...
 if(InpTrendFilter<0 || InpTrendFilter>1 || InpTrendEMA<2 || InpMinDailyADX<0)return INIT_PARAMETERS_INCORRECT;
 if(InpTrendFilter==1){h_ema=iMA(_Symbol,PERIOD_D1,InpTrendEMA,0,MODE_EMA,PRICE_CLOSE);if(h_ema==INVALID_HANDLE)return INIT_FAILED;}
 if(InpMinDailyADX>0){h_adx=iADX(_Symbol,PERIOD_D1,14);if(h_adx==INVALID_HANDLE)return INIT_FAILED;}
...
void OnDeinit(const int reason) {
 ...
 PrintFormat("ONVT_SUMMARY|%s|days=%d|profiles=%d|missing=%d|first_inside=%d|signals=%d|entries=%d|skipped_open=%d|invalid_geometry=%d|rejected=%d|trail_moves=%d|trail_market_exits=%d|trail_rejects=%d|target_behind_entry=%d|trend_filtered=%d|adx_filtered=%d|min_equity=%.2f|max_equity_dd=%.6f|balance=%.2f",
  InpCase,days,profiles,missing,inside_first,signals,entries,skipped_open,geometry,rejected,trail_moves,trail_market_exits,trail_rejects,target_invalid,trend_filtered,adx_filtered,min_equity,max_dd,AccountInfoDouble(ACCOUNT_BALANCE));
}
