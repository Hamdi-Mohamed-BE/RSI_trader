...
Adapted from US100 H1 ORB ADX RR1 Research 2026-09-23/parse_native.py (no time exit here, so hold times are reported).
...
FLAT_UTC_MINUTES = 24 * 60  # no time exit in this strategy; late-exit column is not meaningful
...
def main():
    out, ledgers = {}, {}
    for run in sorted((ROOT / "native").glob("onvt-*/run.json")):
        ...
        for x in tr:
            e = datetime.strptime(x["entry"], "%Y.%m.%d %H:%M:%S")
            q = datetime.strptime(x["exit"], "%Y.%m.%d %H:%M:%S")
            holds.append((q - e).total_seconds() / 60)
        holds.sort()
        s.update(name=meta["name"], symbol=meta["symbol"], mode=meta["mode"], variant=meta.get("variant", "v100"),
                 mode_label=CONFIG["entry_modes"][meta["mode"]]["label"],
                 journal_flags=meta["journal_flags"], ea_summary=meta["ea_summary"],
                 median_hold_minutes=round(holds[len(holds) // 2], 1) if holds else None,
                 max_hold_hours=round(holds[-1] / 60, 1) if holds else None,
                 holds_over_1_day=sum(h > 1440 for h in holds))
        out[meta["name"]] = s
        ledgers[meta["name"]] = tr
    (ROOT / "NATIVE_RESULTS.json").write_text(json.dumps(out, indent=1), encoding="utf-8")
    (ROOT / "native" / "trades.json").write_text(json.dumps(ledgers, indent=1), encoding="utf-8")
    for k, s in out.items():
        print(f"{k:<16} trades {s['trades']:>3}  win {s['win_rate_pct']:>5}%  PF {s['profit_factor']}  ret {s['return_pct']:>7}%  "
              f"eqDD {s['equity_dd_rel']}  balDD {s['balance_dd_rel']}  streak W{s['max_win_streak']}/L{s['max_loss_streak']}  "
              f"avgW {s['avg_win']} avgL {s['avg_loss']}  hold med {s['median_hold_minutes']}m max {s['max_hold_hours']}h  "
              f">1d {s['holds_over_1_day']}  comm {s['commission']} swap {s['swap']}  q {s['quality']}")
