    ...
    for item in catalog:
        ...
        for alias_index, original_alias in enumerate(aliases):
            ...
            if normalized == alias:
                ...
                continue
            path_penalty = 0 if ("share" in item.path.lower() or "indice" in item.path.lower()) else 20
            futures_penalty = 10_000 if "future" in item.path.lower() else 0
            score = match * 1_000 + alias_index * 10 + len(normalized) - len(alias) + path_penalty + futures_penalty
            if score < best_score:
...
def main() -> int:
    ...
    try:
        ...
        for label, aliases in CANDIDATES.items():
            ...
            if catalog_item is None:
                manifest["instruments"][label] = {
                    ...
                    "message": "No enabled broker symbol matched; skipped without aborting.",
                }
            ...
            if not mt5.symbol_select(symbol, True):
                manifest["instruments"][label] = {
                    ...
                    "message": f"Broker symbol exists but could not be selected: {mt5.last_error()}",
                }
            ...
            for year in range(START_YEAR, END.year + 1):
                destination = DATA / f"MEXAtlantic-{label}-{safe_symbol(symbol)}-M1-{year}.csv.gz"
            ...
            complete = not missing_years and len(files) == END.year - START_YEAR + 1
            manifest["instruments"][label] = {
                ...
                "median_spread_price": float(spread_values.median() * info.point) if len(spread_values) else 0.0,
        ...
        ready = [key for key, value in manifest["instruments"].items() if value["status"] == "READY"]
        skipped = [key for key, value in manifest["instruments"].items() if value["status"] != "READY"]
