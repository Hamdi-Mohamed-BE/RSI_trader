"""Native MT5 Strategy Tester comparison: Nasdaq 5M Candle Momentum current vs DI-agreement filter.
...
def run_case(version: str, period: str, expert_file: str, set_name: str, set_body: str) -> dict:
    ...
    case = f"n5-{version}-{period}-m{CONFIG['model']}-d{CONFIG['execution_delay_ms']}"
    out = OUT / case
    out.mkdir(parents=True, exist_ok=True)
...
ToDate={end}
ForwardMode=0
Report=reports\\calyx-n5-di-compare\\{case}.htm
ReplaceReport=1
ShutdownTerminal=1
UseLocal=1
UseRemote=0
...
Visual=0
""", encoding="utf-8-sig")
    (TESTER / "reports" / "calyx-n5-di-compare").mkdir(parents=True, exist_ok=True)
    report = TESTER / "reports" / "calyx-n5-di-compare" / f"{case}.htm"
    if isolated_running():
...
def main() -> None:
    ...
    cur_ex5 = PACKAGE / CONFIG["current_ex5"]
    new_ex5 = (ROOT / CONFIG["new_source"]).with_suffix(".ex5")
    shutil.copy2(cur_ex5, dest / CONFIG["current_expert_file"])
    shutil.copy2(new_ex5, dest / CONFIG["new_expert_file"])
    base_set = PACKAGE / CONFIG["current_set"]
    tester_overrides = CONFIG["tester_overrides"]
    cur_body = set_text(base_set, tester_overrides)
    new_body = set_text(base_set, {**tester_overrides, **CONFIG["new_overrides"]})
    build.update(current_ex5=str(cur_ex5), current_ex5_sha256=sha(cur_ex5), base_set=str(base_set), base_set_sha256=sha(base_set))
    (OUT / "build.json").write_text(json.dumps(build, indent=2), encoding="utf-8")
    only = sys.argv[1:]  # optional period keys, e.g. "since2021"; default = all
    for k in only:
        if k not in CONFIG["periods"]:
            fail(f"Unknown period key {k}")
    results = []
    for period in [p for p in CONFIG["periods"] if not only or p in only]:
        results.append(run_case("current", period, CONFIG["current_expert_file"], "n5-compare-current.set", cur_body))
        results.append(run_case("di", period, CONFIG["new_expert_file"], "n5-compare-di.set", new_body))
    (OUT / ("runs.json" if not only else f"runs-{'-'.join(only)}.json")).write_text(json.dumps(results, indent=2), encoding="utf-8")
