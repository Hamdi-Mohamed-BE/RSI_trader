...
def field(item, name: str, default=0.0):
    """Read a symbol field across old and new MetaTrader5 Python packages."""
...
def symbol_payload(original, account_currency: str) -> dict:
    ...
    bid = max(float(field(item, "bid")), float(field(tick, "bid")))
    ask = max(float(field(item, "ask")), float(field(tick, "ask")))
    last = max(float(field(item, "last")), float(field(tick, "last")))
    reference_price = max(
        bid,
        ask,
        last,
        float(field(item, "session_price_open")),
        float(field(item, "session_price_settlement")),
    )
    ...
    tick_size = abs(float(field(item, "trade_tick_size"))) or abs(float(field(item, "point")))
    tick_value = max(
        abs(float(field(item, "trade_tick_value"))),
        abs(float(field(item, "trade_tick_value_loss"))),
        abs(float(field(item, "trade_tick_value_profit"))),
    )

    if is_target_symbol(item.name) and tick_value <= 0 and reference_price > 0 and tick_size > 0:
        calculation_volume = max(float(field(item, "volume_min")), min(1.0, float(field(item, "volume_max", 1.0))))
        profit_calculator = getattr(mt5, "order_calc_profit", None)
        calculated_profit = None
        if callable(profit_calculator):
            calculated_profit = profit_calculator(
                mt5.ORDER_TYPE_BUY,
                item.name,
                calculation_volume,
                reference_price,
    ...
    if (
        ...
        and tick_value <= 0
        and tick_size > 0
        and float(field(item, "trade_contract_size")) > 0
        and str(field(item, "currency_profit", "")).upper() == account_currency.upper()
    ):
        tick_value = abs(float(field(item, "trade_contract_size")) * tick_size)
    ...
    return {
        ...
        "volume_min": float(field(item, "volume_min")),
        "volume_max": float(field(item, "volume_max")),
        "volume_step": float(field(item, "volume_step")),
        "bid": bid,
        "ask": ask,
        ...
        "trade_contract_size": float(field(item, "trade_contract_size")),
        "currency_profit": str(field(item, "currency_profit", "")),
    }
