...
bool observedNonpositive=false;
void Track()
{
 double e=A(ACCOUNT_EQUITY),total=0;PositionCount(total);
 if(!observedNonpositive && e<=0 && total>0){observedNonpositive=true;Event("observed_nonpositive_equity","first nonpositive equity at tick; observer only");}
 if(peakEq>0 && 100*(peakEq-e)/peakEq>maxDD && 100*(peakEq-e)/peakEq>=100)Event("dd_over_100_record","highwater="+DoubleToString(peakEq,2)+"; dd_pct="+DoubleToString(100*(peakEq-e)/peakEq,8));
