...
def simulate(
    ...
    for signal in pivot_signals(frame, distance):
...
def metrics(
    ...
    records = list(trades)
    results = [trade.result_r for trade in records]
    ...
    for value in results:
        balance += balance * risk_pct / 100.0 * value
    ...
    return {
        ...
        "wins": len(wins),
        "losses": len(losses),
        "win_rate_pct": len(wins) / len(records) * 100.0 if records else 0.0,
        "net_r": sum(results),
        "profit_factor": gross_profit / gross_loss if gross_loss else math.inf,
        "expectancy_r": sum(results) / len(records) if records else 0.0,
    ...
    try:
        ...
        for requested_symbol in requested:
            try:
                ...
                frame = completed_h4_rates(symbol, start, end, distance)
