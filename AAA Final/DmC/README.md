...
6. Use the instrument profile for the initial stop: 45 index points on US100 or $22.50 on gold. Start trailing after +2R with a 1.5R distance.
...
Gold is enabled through the same D1/H4 plus H1 body-retest logic. `XAUUSD` is automatically resolved to the connected broker's gold alias. Each market risks 2%, while `MAX_TOTAL_RISK_PCT=4` limits the DmC basket to two simultaneous ideas.
