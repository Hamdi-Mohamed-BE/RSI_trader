...
def place_pending(
    ...
    comment = f"AMD A NY {signal.side.upper()} stop"[:31]
...
def place_article_market(
    ...
    request: dict[str, object] = {
        ...
        "magic": config.magic,
        "comment": (
            f"AMD A+ {phase_code} {signal.side.upper()} "
            f"{'session fade' if phase_code == 'AF' else 'distribution'}"
        )[:31],
        "type_time": mt5.ORDER_TIME_GTC,
    }
    actual_risk_pct = planned / float(account.equity) * 100.0
...
def stale_market_reason(
    frame: pd.DataFrame,
    now: datetime,
    *,
    max_age: timedelta = timedelta(minutes=15),
) -> str | None:
    """Report a closed/stale feed before strategy diagnostics run."""
    if frame.empty or "time" not in frame:
        return "market data unavailable"
    latest = pd.Timestamp(frame["time"].max())
    latest = (
        latest.tz_localize("UTC")
        if latest.tzinfo is None
        else latest.tz_convert("UTC")
    )
    ...
    if current - latest > max_age:
        return (
            "market closed or feed stale; latest M1 bar "
            f"{latest:%Y-%m-%d %H:%M} UTC"
        )
    return None
        ...
        with connection() as account:
            ...
            for canonical, symbol in symbols.items():
                try:
                    ...
                    stale_reason = stale_market_reason(frame, now)
                    if stale_reason is not None:
                        print(f"  {canonical:<7} {stale_reason}")
                        continue
