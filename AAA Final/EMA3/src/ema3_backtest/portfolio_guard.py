...
def _risk_by_magic() -> dict[int, float]:
    ...
    for item in os.getenv("SELECTED_XAU_MAGIC_RISKS", "").split(","):
        if not item.strip():
            continue
...
def _active_reserved_risk_pct(risk_by_magic: dict[int, float]) -> float:
    ...
    if positions is None or orders is None:
        raise RuntimeError(f"Could not read MT5 portfolio exposure: {mt5.last_error()}")
    active_magics = {
...
@contextmanager
def selected_xau_entry_guard(proposed_risk_pct: float) -> Iterator[PortfolioDecision]:
    ...
    if cap <= 0 or not risk_by_magic:
        ...
        return
    lock_path = Path(__file__).resolve().parents[3] / "runtime" / "selected-xau-portfolio.lock"
    ...
    while descriptor is None:
        try:
            ...
            if time.monotonic() >= deadline:
                raise RuntimeError("Timed out waiting for the shared XAU risk lock")
            time.sleep(0.1)
    try:
