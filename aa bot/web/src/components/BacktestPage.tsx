...
interface BacktestTrade { id: number; direction: "long" | "short"; grade: "A" | "A+"; model: "EM1" | "EM2" | "EM3" | "EM4"; entryTime: number; exitTime: number; entry: number; stop: number; target: number; exit: number; resultR: number; pnl: number; exitReason: string; holdBars: number }
...
export function BacktestPage({ products, defaultAccount = 50_000, defaultRisk = 0.5 }: { products: string[]; defaultAccount?: number; defaultRisk?: number }) {
  const optimizedMarket = products.includes("ETH-USD") ? "ETH-USD" : products[0] ?? "BTC-USD";
  const [form, setForm] = useState({ productId: optimizedMarket, granularity: 3600, bars: 1440, direction: "short", model: "all", quality: "A", lookback: "24", targetR: "2", riskPct: String(defaultRisk), accountSize: String(defaultAccount), maxHoldBars: "120" });
  ...
  return <div className="page-stack">
    <section className="panel backtest-hero"><div><span className="eyebrow">Research laboratory · 60-day optimized setup</span><h2>LTA entry-model backtest</h2><p>Default: ETH H1 shorts · all EM models · A/A+ · 24-bar profile · 2R minimum · 120-bar maximum hold.</p></div><div className="research-warning"><ShieldAlert/><span><strong>Candle approximation</strong><em>Historical MBO is not available until the recorder builds a local dataset.</em></span></div></section>
    <section className="panel backtest-controls"><div className="panel-head compact"><div><span className="eyebrow">Experiment</span><h3><FlaskConical size={17}/> Parameters</h3></div><button className="primary run-button" onClick={run} disabled={loading}>{loading ? <LoaderCircle className="spin"/> : <Play/>}{loading ? "Running…" : "Run backtest"}</button></div>
      <div className="backtest-fields">
        ...
        <label><span>History</span><select value={form.bars} onChange={(e) => set("bars", Number(e.target.value))}><option value="300">300 bars</option><option value="600">600 bars</option><option value="1200">1,200 bars</option><option value="1440">1,440 bars · 60 days on H1</option></select></label>
        ...
        <label><span>Entry model</span><select value={form.model} onChange={(e) => set("model", e.target.value)}><option value="all">All EM models</option><option>EM1</option><option>EM2</option><option>EM3</option><option>EM4</option></select></label>
        <label><span>Quality</span><select value={form.quality} onChange={(e) => set("quality", e.target.value)}><option value="A+">A+ only</option><option value="A">A and A+</option></select></label>
        <label><span>Profile lookback</span><input type="number" min="20" max="120" value={form.lookback} onChange={(e) => set("lookback", e.target.value)}/></label>
        <label><span>Minimum target R</span><input type="number" min="1.5" max="5" step="0.5" value={form.targetR} onChange={(e) => set("targetR", e.target.value)}/></label>
